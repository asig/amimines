...
std::unordered_map<std::string, Type> typeNameToVal = {
    {"DISK", Type::DISK},       {"DRAWER", Type::DRAWER},
    {"TOOL", Type::TOOL},       {"PROJECT", Type::PROJECT},
    {"GARBAGE", Type::GARBAGE}, {"DEVICE", Type::DEVICE},
    {"KICK", Type::KICK},       {"APPICON", Type::APPICON},
};
...
public:
  ...
  void checkPos(std::uint32_t pos) {
    if (pos_ != pos) {
      std::cerr << "BAD POS: Expected " << pos << ", actual " << pos_
                << std::endl;
      abort();
    }
  }
  ...
  binstream &operator<<(std::uint32_t v) {
    os_ << (std::uint8_t)(v >> 24 & 0xff) << (std::uint8_t)(v >> 16 & 0xff)
        << (std::uint8_t)(v >> 8 & 0xff) << (std::uint8_t)(v & 0xff);
    pos_ += 4;
    return *this;
  }
  ...
  binstream &operator<<(std::uint16_t v) {
    os_ << (std::uint8_t)(v >> 8 & 0xff) << (std::uint8_t)(v & 0xff);
    pos_ += 2;
    return *this;
  }
...
void usage() {
  std::cerr
      << "usage: infogen --type <type> [--stacksize <stacksize>] {--icon "
         "path/to/icon@<rect>} [--x int] [--y int] [--drawer <rect>] "
         "[--default_tool string] [--tool_window] info-file-name"
      << "\n"
      << "   where\n"
      << "      <type> is one of DISK, DRAWER, TOOL, PROJECT, GARBAGE, DEVICE, "
         "KICK, or APPICON"
      << "\n"
      << "      <stacksize> is an integer > 4096"
      << "\n"
      << "      <rect> is a comma-separated list of for ints, specifing a "
         "rectangle (x,y,w,h)"
      << "\n"
      << "   --icon needs to be present at least once, and at most 2 times."
      << "\n"
      << "      <rect> specifies the part of the image that should be used, "
         "and is expressed as x,y,w,h ."
      << "\n"
      << "      Example: to extract 3232 pixels at the position (100,150) from "
         "foo.iff, use: --icon foo.iff@100,150,32,32\n";
  exit(1);
}
...
int main(int argc, char *const *argv) {
  ...
  //         bit 0                   if set we use backfill mode, else
  //         complement mode
  ...
  os << (std::uint32_t)(
      images.size() > 1 ? 0x1234
                        : 0); // byte order doesn't matter as long as it's != 0
  ...
  //    0x28 APTR  ga_UserData       lower 8 bits:  0 for old, 1 for icons >=
  //    OS2.x
  ...
  if (hasDrawerData) {
    ...
    //     0x26 WORD  nw_MinWidth       <undefined> often 94, minimum window
    //     width
    os << (std::int16_t)94; // FIXME DO NOT HARDCODE

    //     0x28 WORD  nw_MinHeight      <undefined> often 65, minimum window
    //     height
    os << (std::int16_t)65; // FIXME DO NOT HARDCODE

    //     0x2A UWORD nw_MaxWidth       <undefined> often 0xFFFF, maximum window
    //     width
    os << (std::uint16_t)0xffff;

    //     0x2C UWORD nw_MaxHeight      <undefined> often 0xFFFF, maximum window
    //     width
    os << (std::uint16_t)0xffff;
  ...
  if (hasDefaultTool) {
    // 0x00 ULONG tx_Size           the size of tx_Text including zero byte
    // (tx_Zero)
