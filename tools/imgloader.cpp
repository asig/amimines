...
Image::Image() : w_(0), h_(0), depth_(0), lineBytes_(0) {}

Image::Image(int w, int h, int depth, int lineBytes,
...
IffLoader::IffLoader(FILE *fp) : fp_(fp) {
  uint8_t buf[5];
  buf[4] = 0;
  ...
  while (remainingSize > 0) {
    ...
    if (std::memcmp("BMHD", buf, 4) == 0) {
      ...
      readBODY();
      // } else {
      //    std::cout << "Ignoring chunk " << static_cast<unsigned char*>(buf)
      //    << std::endl;
    }
    if (sz & 1) {
      // skip padding
...
void IffLoader::readBMHD() {
  ...
  skip(1 + 2 + 1 + 1 + 2 +
       2); // pad, transClr, xAspect, yAspect, pageWidth, pageHeight
}
...
void IffLoader::skip(long b) { fseek(fp_, b, SEEK_CUR); }
...
std::uint8_t IffLoader::readUint8() { return (std::uint8_t)fgetc(fp_); }
...
PngLoader::PngLoader(FILE *fp) {
  ...
  if (colorType != PNG_COLOR_TYPE_PALETTE) {
    throw std::runtime_error("Unsupported colorType " +
                             std::to_string(colorType) +
                             ", only PNG_COLOR_TYPE_PALETTE is supported");
  }
...
bool PngLoader::canLoad(FILE *fp) {
  static std::uint8_t pngMagic[] = {0x89, 0x50, 0x4E, 0x47,
                                    0x0D, 0x0A, 0x1A, 0x0A};
