...
struct Image *tileWithCounts[] = {
    &imgTile0, &imgTile1, &imgTile2, &imgTile3, &imgTile4,
    &imgTile5, &imgTile6, &imgTile7, &imgTile8,
};

void drawRemainingMines(int mines) {
  if (mines < 0) {
    DrawImage(window->RPort, &imgDigitDash, REMAINING_MINES_X,
              REMAINING_MINES_Y);
    mines = -mines;
    DrawImage(window->RPort, digits[mines / 10],
              REMAINING_MINES_X + 1 * imgDigit0.Width, REMAINING_MINES_Y);
    DrawImage(window->RPort, digits[mines % 10],
              REMAINING_MINES_X + 2 * imgDigit0.Width, REMAINING_MINES_Y);
  } else {
    DrawImage(window->RPort, digits[mines / 100],
              REMAINING_MINES_X + 0 * imgDigit0.Width, REMAINING_MINES_Y);
    mines %= 100;
    DrawImage(window->RPort, digits[mines / 10],
              REMAINING_MINES_X + 1 * imgDigit0.Width, REMAINING_MINES_Y);
    DrawImage(window->RPort, digits[mines % 10],
...
void drawTimer(int secs) {
  secs = secs % 1000;
  DrawImage(window->RPort, digits[secs / 100], TIMER_X + 0 * imgDigit0.Width,
            TIMER_Y);
  secs %= 100;
  DrawImage(window->RPort, digits[secs / 10], TIMER_X + 1 * imgDigit0.Width,
            TIMER_Y);
  DrawImage(window->RPort, digits[secs % 10], TIMER_X + 2 * imgDigit0.Width,
            TIMER_Y);
}

void drawPlayfieldTileExplicit(USHORT x, USHORT y, struct Image *img) {
  DrawImage(window->RPort, img, PLAYFIELD_X + (x - 1) * 16,
            PLAYFIELD_Y + (y - 1) * 16);
}
...
BOOL mouseToTile(SHORT mouseX, SHORT mouseY, USHORT *tileX, USHORT *tileY) {
  mouseX -= PLAYFIELD_X;
  mouseY -= PLAYFIELD_Y;
  if (mouseX < 0 || mouseX >= PLAYFIELD_W_TILES * imgTile0_W || mouseY < 0 ||
      mouseY >= PLAYFIELD_H_TILES * imgTile0_H) {
    // outside playfield
    *tileX = *tileY = 0;
    return FALSE;
  }
...
void diedAt(USHORT tx, USHORT ty) {
  ...
  for (USHORT y = 1; y < PLAYFIELD_H_TILES + 1; y++) {
    for (USHORT x = 1; x < PLAYFIELD_W_TILES + 1; x++) {
      ...
      } else if ((t->state == TILE_CLOSED || t->state == TILE_MARKED_UNKNOWN) &&
                 t->mine) {
...
void openEmpty(USHORT x, USHORT y) {
  struct Tile *tile = &game.tiles[y][x];
  if (tile->state == TILE_OPEN || x < 1 || x > PLAYFIELD_W_TILES || y < 1 ||
      y > PLAYFIELD_H_TILES) {
    return;
  }
