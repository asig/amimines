...
static struct Gadget checkboxExpertGadget = {
    NULL, /* NextGadget */
    LOGO_X + 114,
    LOGO_Y + 31, /* LeftEdge, TopEdge */
    checkboxUnselected_W,
    checkboxUnselected_H,      /* Width, Height */
...
static struct Gadget faceGadget = {
    ...
    SMILEY_X,
    SMILEY_Y, /* LeftEdge, TopEdge */
    imgFaceNormal_W,
    imgFaceNormal_H,        /* Width, Height */
    GADGIMAGE | GADGHIMAGE, /* Flags */
    RELVERIFY,              /* Activation */
    BOOLGADGET,             /* Gadget Type */
...
static struct NewWindow newWindow = {
    ...
    ACTIVATE | BORDERLESS |
        RMBTRAP, // | SMART_REFRESH | ACTIVATE | WINDOWSIZING | SIZEBRIGHT |
                 // WINDOWDRAG | WINDOWDEPTH | BORDERLESS,
    &faceGadget,
    NULL,
    NULL, // Title
    NULL, // Screen
...
void border(struct RastPort *rp, int x, int y, int w, int h, int d,
            int colorTopLeft, int colorBottomRight, int colorBetween) {
  int x1 = x;
  int y1 = y;
  int x2 = x + w - 1;
  int y2 = y + h - 1;
...
void uiCreate(int startDifficulty) {
  ...
  // Draw header part
  border(window->RPort, HEADER_X, HEADER_Y, HEADER_W, HEADER_H, HEADER_BORDER,
         COL_DGRAY, COL_LGRAY, COL_GRAY);
  border(window->RPort, REMAINING_MINES_X - 1, REMAINING_MINES_Y - 1, 39 + 2,
         23 + 2, 1, COL_DGRAY, COL_LGRAY, COL_GRAY);
  border(window->RPort, TIMER_X - 1, TIMER_Y - 1, 39 + 2, 23 + 2, 1, COL_DGRAY,
         COL_LGRAY, COL_GRAY);
  DrawImage(window->RPort, &imgFaceNormal, SMILEY_X, SMILEY_Y);

  // Draw Logo part
  DrawImage(window->RPort, &imgLogo, LOGO_X, LOGO_Y);
  // border(window->RPort, LOGO_X, LOGO_Y, LOGO_W, LOGO_H, LOGO_BORDER,
  // COL_DGRAY, COL_LGRAY, COL_GRAY);

  // Draw playfield border and playfield
  border(window->RPort, PLAYFIELD_X - PLAYFIELD_BORDER,
         PLAYFIELD_Y - PLAYFIELD_BORDER,
         PLAYFIELD_W_TILES * 16 + 2 * PLAYFIELD_BORDER,
         PLAYFIELD_H_TILES * 16 + 2 * PLAYFIELD_BORDER, PLAYFIELD_BORDER,
         COL_DGRAY, COL_LGRAY, COL_GRAY);

  uiDifficultyRadioGroup.gadgets = difficultyGadgets;
  uiDifficultyRadioGroup.nofGadgets =
      sizeof(difficultyGadgets) / sizeof(struct Gadget *);
